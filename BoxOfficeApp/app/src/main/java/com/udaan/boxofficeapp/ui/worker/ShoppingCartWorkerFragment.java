package com.udaan.boxofficeapp.ui.worker;
...
public class ShoppingCartWorkerFragment extends Fragment {
    ...
    private class AddToCartTask extends AsyncTask<String, Void, String> {
        ...
        private void saveAddedItemToDatabase(long serverId, String productCode, String productId, String quantity) {
            ...
            values.put(BoxOfficeContract.CartEntry.COLUMN_SERVER_ID, serverId);
            values.put(BoxOfficeContract.CartEntry.COLUMN_PRODUCT_ID, productId);
//            values.put(BoxOfficeContract.CartEntry.COLUMN_COLOR_SERVER_ID, colorServerId);
//            values.put(BoxOfficeContract.CartEntry.COLUMN_COLOR, color);
//            values.put(BoxOfficeContract.CartEntry.COLUMN_SIZE_SERVER_ID, sizeServerId);
//            values.put(BoxOfficeContract.CartEntry.COLUMN_SIZE, size);
            values.put(BoxOfficeContract.CartEntry.COLUMN_QUANTITY, quantity);
            values.put(BoxOfficeContract.CartEntry.COLUMN_DATE_ADDED, DateUtils.getCurrentRfc3339Date());

            getContext().getContentResolver().insert(BoxOfficeContract.CartEntry.CONTENT_URI, values);
        }
    }
    ...
    private class RemoveFromCartTask extends AsyncTask<String, Void, String> {
        ...
        private void removeItemFromDatabase(String cartItemId) {
            getContext().getContentResolver().delete(
                    BoxOfficeContract.CartEntry.buildCartItemUri(Long.parseLong(cartItemId)), null, null);
        }
    ...
    private class UpdateCartItemTask extends AsyncTask<String, Void, String> {
        ...
        private void updateItemQuantityLocally(String cartItemId, String newQuantity) {
            ContentValues values = new ContentValues();
            values.put(BoxOfficeContract.CartEntry.COLUMN_QUANTITY, newQuantity);

            getContext().getContentResolver().update(
                    BoxOfficeContract.CartEntry.buildCartItemUri(Long.parseLong(cartItemId)),
                            values, null, null);
        }
    }
}
